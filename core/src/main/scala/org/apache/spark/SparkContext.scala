...
import org.apache.spark.scheduler.eagle.{EagleScheduler, EagleSchedulerBackend}
import org.apache.spark.scheduler.pigeon.{PigeonScheduler, PigeonSchedulerBackend}
...
object SparkContext extends Logging {
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      case PIGEON_REGEX(host, port) =>
        val scheduler = new PigeonScheduler(sc, host, port, System.getProperty("pigeon.app.name", "spark"))
        val backend = new PigeonSchedulerBackend(scheduler)
        scheduler.initialize(backend)
        (backend, scheduler)
...
private object SparkMasterRegex {
  ...
  // Pigeon plugin
  val PIGEON_REGEX = """pigeon@([A-Za-z0-9\.]+):([0-9]+)[,.*]*""".r
}
